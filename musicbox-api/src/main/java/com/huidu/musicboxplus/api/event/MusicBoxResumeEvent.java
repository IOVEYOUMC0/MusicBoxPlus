...
// Fired when a player resumes playback from the paused state.
// Cancelling it with setCancelled(true) aborts the resume, leaving playback paused.
public class MusicBoxResumeEvent extends Event implements Cancellable {
    ...
    public MusicBoxResumeEvent(MusicBoxSongPlayer player) {
        this.player = player;
    ...
    // The song playing at the moment of the resume; null when the player has no current song.
    ...
    // Null for non-positional players; getWorld() on the returned Location may be null once the
